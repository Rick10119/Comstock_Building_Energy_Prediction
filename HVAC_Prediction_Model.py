...
#upgrade_selected_data_df = pd.read_parquet('C:/Users/3.22/VS Code 2024/Demand-Response-Potential-Assessment-through-AI-and-Simulation-main/downloaded_data/selected_data_new_upgrade1_seasons_CNN.parquet')
upgrade_selected_data_df = pd.read_parquet('C:/Users/3.22/VS Code 2024/Demand-Response-Potential-Assessment-through-AI-and-Simulation-main/downloaded_data/selected_data_new_upgrade1_seasons_CNN_Pred.parquet')

#Clean unused features
unused_features = [#'out.electricity.total.energy_consumption','augmented_baseload',
                    'bldg_id','in.airtightness..m3_per_m2_s','applicability','in.building_subtype',
                    'in.wall_construction_type','in.window_to_wall_ratio_category','in.window_type',
                    'in.weekday_opening_time..hr','in.weekday_operating_hours..hr','in.weekend_opening_time..hr','in.weekend_operating_hours..hr']
upgrade_selected_data_df = upgrade_selected_data_df.drop(columns=unused_features)
str_to_num_col = ['in.aspect_ratio','in.number_of_stories',
                  #'in.weekday_opening_time..hr','in.weekday_operating_hours..hr','in.weekend_opening_time..hr','in.weekend_operating_hours..hr',
                  'in.year_built']


# 提取特征和标签
#X = upgrade_selected_data_df[upgrade_selected_data_df['season'] == 'summer']
X= upgrade_selected_data_df
...
X = X.drop(columns = ['Global Horizontal Radiation [W/m2]','Direct Normal Radiation [W/m2]','Diffuse Horizontal Radiation [W/m2]','out.electricity.cooling.energy_consumption'])
y = upgrade_selected_data_df['out.electricity.cooling.energy_consumption']
...
X_seq, y_seq = create_sequences(X, y, seq_length=24)
print(X_seq.shape[2])
...
class HVACCNN(nn.Module):
    def __init__(self, input_shape, output_length):
        ...
        self.conv1 = nn.Conv2d(1, 2048, kernel_size=(2, 2), padding='valid') #128
        self.bn1 = nn.BatchNorm2d(2048)
        self.conv2 = nn.Conv2d(2048, 2048, kernel_size=(2, 1), padding='valid') #256
        self.bn2 = nn.BatchNorm2d(2048)
        self.conv3 = nn.Conv2d(2048, 2048, kernel_size=(2, 1), padding='valid') #256
        self.bn3 = nn.BatchNorm2d(2048)
        self.conv4 = nn.Conv2d(2048,2048, kernel_size=(2, 1), padding='valid') #256
        self.bn4 = nn.BatchNorm2d(2048)
        self.conv5 = nn.Conv2d(2048,2048, kernel_size=(2, 1), padding='valid') #256
        self.bn5 = nn.BatchNorm2d(2048)
        self.conv6 = nn.Conv2d(2048, 1024, kernel_size=(2, 1), padding='valid') #256
        self.bn6 = nn.BatchNorm2d(1024)
        self.conv7 = nn.Conv2d(1024, 1024, kernel_size=(2, 1), padding='valid') #256
        self.bn7 = nn.BatchNorm2d(1024)
        self.conv8 = nn.Conv2d(1024, 1024, kernel_size=(2, 1), padding='valid') #256
...
model = HVACCNN(input_shape, output_length=24).to(device)
...
def train_model(model, train_loader, test_loader, criterion, optimizer, epochs):
    ...
    for epoch in range(epochs):
        ...
        if epoch_val_loss < best_loss:
            ...
            no_improve_epochs = 0
            torch.save(model.state_dict(), 'best_model_HVAC.pth')
...
# Train the model
train_model(model, train_loader, test_loader, criterion, optimizer, epochs=45)

# Load best model
model.load_state_dict(torch.load('best_model_HVAC.pth'))
model.eval()
...
# 11. Plot example
import matplotlib.pyplot as plt
#plt.plot(X_test1[0,:,:24,-1], label='Base Load', linestyle='--')
plt.plot(y_test[0,:24], label='Actual HVAC',linewidth=2)
plt.plot(y_test_pred[0,:24], label='Predicted HVAC',linewidth=2)
plt.xlabel('Number of Buildings')
plt.ylabel('Cooling Electricity Load (kwh)')
plt.title('Buildings HVAC Load Forecasting Graph (NY State)')
plt.legend()
plt.grid(True)
plt.show()